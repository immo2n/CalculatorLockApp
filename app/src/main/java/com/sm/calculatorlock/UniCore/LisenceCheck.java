package com.sm.calculatorlock.UniCore;
        ...
        this.mainDB.collection(LISENCE_COLLECTION).document(LISENCE_DOCUMENT).get().addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                if (task.getResult().exists()) {
                    ...
                    MaterialAlertDialogBuilder builder = new MaterialAlertDialogBuilder(context);
                    builder.setTitle("App verification failed!")
                            .setMessage("Unable to verify the application! Please update the app from respective stores.")
                            .setCancelable(false)
                            .setNegativeButton("OK, understood", (dialog, which) -> {
                                dialog.dismiss();
                                activity.finish();
                            })
                            .show();
